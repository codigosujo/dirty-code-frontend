                ...
                {selectedAction && (
                    <div className="animate-in fade-in slide-in-from-top-4 duration-300">
                        <div className={`
                            ...
                            ${meetsRequirements(selectedAction) ? 'bg-zinc-900/40 border-green-500/50' : 'bg-red-950/10 border-red-500/30'}
                            shadow-[0_0_30px_rgba(0,0,0,0.3)]
                        `}>
                            <div className="flex flex-col gap-6">
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between">
                                        <h3 className={`text-2xl font-bold ${meetsRequirements(selectedAction) ? 'text-green-500' : 'text-red-400'}`}>
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm font-mono bg-black/40 p-4 rounded-xl border border-white/5">
                                    ...
                                    <div className="flex flex-col gap-1">
                                        <span className="text-gray-500 uppercase tracking-wider text-[10px]">STL Necessário</span>
                                        <div className="flex items-baseline gap-2">
                                            <span className={`text-xl font-bold ${(user?.activeAvatar?.stealth || 0) >= selectedAction.requirements.stealth ? 'text-white' : 'text-red-500'}`}>
                                                {selectedAction.requirements.stealth}
                                            </span>
                                        </div>
                                    </div>
                                    <div className="flex flex-col gap-1">
